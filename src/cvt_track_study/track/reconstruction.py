...
def build_track_evidence(
    ...
    lap_gate = _find_lap_gate(
        events, settings.lap_gate_event_id
    )
    all_points = _combine_points(
        ingestion_results
    )
    frame = LocalFrame(
    ...
    laps = detect_laps(
        ...
        frame,
        float(
            lap_gate["anchor_latitude_deg"]
        ),
        float(
            lap_gate["anchor_longitude_deg"]
        ),
        settings,
        diagnostics,
    )
    ...
    rejected_map_points = (
        consensus.rejected_map_points
    )

    if not laps["analysis_valid"].any():
        raise ValueError(
            "No valid map-matched laps remain after "
...
def _combine_points(
    ...
    for result in results:
        ...
        frame["use_for_centreline"] = (
            result.metadata.use_for_centreline
        )
        frame["use_for_gate_evidence"] = (
            result.metadata.use_for_gate_evidence
        )
        frames.append(frame)
    return pd.concat(
