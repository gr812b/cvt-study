...
\hypersetup{
  ...
  pdftitle={Measured Track-Based Drivetrain Design Framework -- Methods},
  pdfauthor={Framework technical documentation}
}
\setlist{nosep}
...
GPX or FIT recordings into drivetrain design evidence.  It explains the complete chain:
configuration provenance, telemetry ingestion, lap reconstruction, centreline and
...
The current ideal-CVT comparison is one reduced-order mechanism inside the measured
track-based framework.  Section~\ref{sec:extensions} states the generic interfaces by
which external tire, obstacle, or mechanism work may consume the reviewed evidence
boundary without rewriting the upstream pipeline.  Detailed physical drivetrain-model
development is outside this repository's scope.
\end{abstract}
...
\begin{description}
  \item[Measured evidence] what was observed in GPX/FIT passes, feature surveys, video, or
...
\centering
Project and profiles $\rightarrow$ canonical telemetry $\rightarrow$ reconstructed laps and
...
bundle, but it may not reinterpret raw telemetry internally. Reporting consumes completed
...
\chapter{Telemetry ingestion}\label{ch:gpx}

\section{Run identity}

Each GPX or FIT file has a unique run identifier plus vehicle and driver identifiers.  The
...
and FIT/GPX exports of one session must not be declared as independent runs.

\section{Safe parsing and canonical telemetry}

The GPX parser uses a hardened XML implementation.  The FIT adapter uses the official
decoder with CRC checking and retains native enhanced speed, cumulative distance,
enhanced altitude, and GPS accuracy when present.  Both preserve timestamps,
coordinates, optional elevation, and source records. Invalid points are diagnosed
rather than interpolated invisibly. Time must be monotonic within a usable segment.
...
records separate native-device, reported, and derived channels plus an analysis-source
and certainty tier. Native FIT speed has first priority, followed by reported GPX speed
and then Equation~\eqref{eq:gpx-speed}. The equation is an observation channel, not a
vehicle dynamic equation.

\section{Diagnostics}

Ingestion reports missing timestamps, coordinate errors, duplicated points, abnormal
time gaps, implausible speed, and derived-speed spikes. Elevation is retained and a
coverage, repeatability, and materiality proxy is exported. It is not differentiated
into road grade automatically: only a material screen can request a paired
with/without-grade sensitivity before a new force is enabled.
...
Each retained telemetry point is projected to the centreline, producing along-track coordinate
...
An accepted entry gate does not automatically become a full-feature speed profile. A
second sustained-response gate is added only when at least $80\%$ of complete passes
show the required entry-to-minimum slowdown, a one-sided exact binomial test gives
$p\leq0.05$, the minimum speed and location are repeatable, and leave-one-lap-out
medians remain stable. Failure retains the entry gate and records an entry-only
fallback rather than inventing a new constraint.
...
For a speed-quadratic event with coefficient $k_v$ and simulated boundary-crossing
entry speed $v_e$,
...
but it may not produce infinite launch torque. For each paired scenario, one launch cap
is frozen from the scenario-level nominal transmission before candidate values are
applied:

\begin{equation}
T_{w,\max}^{\mathrm{launch}}=\eta^{(0)} i_{\max}^{(0)}i_f^{(0)}T_e.
\end{equation}
...
\eqref{eq:bounded-ratio} and \eqref{eq:infinite-torque} define the comparison. Every
candidate in one design-sweep scenario therefore shares the exact reference simulation.
Structural-sensitivity levels retain separate matched references because they test
assumptions rather than competing transmission designs.
...
Grade work $W_g$ is signed. Current telemetry elevation does not contribute to it; a smooth
...
robust sweep uses lap time as the decision metric and additionally requires acceptable
convergence plus a paired lap-time win-fraction bootstrap lower bound above $0.5$. Since
the reference is common within a scenario, bounded lap time and lap-time loss against
that reference have identical rankings. Opportunity energy remains diagnostic and does
not veto that winner. Numerical validity and directional robustness are separate fields.
...
The provenance record connects measured GPX/FIT, track build, exact bundle hash, resolved
...
\section{External mechanism boundary}

An external mechanism may consume the same runtime track, driver demand, vehicle state,
and paired scenario inputs when it declares a versioned configuration and returns
deterministic force, stored-energy, dissipative-loss, and quality channels.  The current
runner uses its built-in ideal comparison directly; the protocol is an API boundary, not
a dynamically selected model system.  Any external implementation requires its own
validated comparison and caching policy rather than inheriting ideal-reference
...
Upstream GPX and bundle code should not contain conditionals for a particular mechanism
family.
...
\begin{itemize}
  \item GPX/FIT elevation is screened but not automatically converted to road grade force.
  ...
  \item The ideal CVT is an intentionally reduced-order comparison mechanism.
...
The implemented grade screen only decides whether a paired spatial-grade sensitivity is
warranted. Enabling grade force still requires smoothing, lap alignment, vertical-bias
handling, and evidence that the sensitivity changes the decision. Obstacle calibration requires known
geometry, video, vehicle/suspension evidence, and controlled before/after speed tests.
Optional expensive global sensitivity may later add Sobol or grouped methods,
...
\begin{enumerate}
  ...
  \item project-validation and track-review evidence blockers are resolved;
